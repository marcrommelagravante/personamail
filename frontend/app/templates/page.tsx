...
export default function TemplatesPage() {
  ...
  const inputClass =
    "rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100";
      ...
      <main className="mx-auto max-w-4xl px-5 py-10 sm:px-8">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">
              Save the messages you send often, then make them personal when
              needed.
            </p>
          </div>
          {isAuthenticated && (
            <button
              ...
              className="rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-500"
            >
        ...
        {!isAuthenticated ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
            <h2 className="font-semibold">Sign in to use templates</h2>
            <p className="mt-2 text-sm text-slate-600">
              Your saved messages stay private to your PersonaMail account.
            </p>
          </div>
        ) : (
          <>
            {showForm && (
              <form
                ...
                className="mb-8 grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:grid-cols-2"
              >
                <input
                  required
                  value={form.name}
                ...
                >
                  <option value="">Suggested tone (optional)</option>
                ...
                <button
                  type="submit"
                  className="rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 sm:col-span-2"
                >
            ...
            {loading ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
                ...
                <p className="mt-2 text-sm text-slate-600">
                  Save a message you send often, so the next version starts
                  closer to done.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex-1">
                      <div className="flex flex-wrap gap-2 text-xs font-medium text-sky-700">
                        {template.relationship && (
                          <span>{template.relationship}</span>
                        )}
                        {template.tone && <span>{template.tone}</span>}
                      </div>
                    ...
                    <div className="mt-5 flex gap-2">
                      ...
                      <button
                        ...
                        className="rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-primary"
                      >
                        Delete
                      </button>
                    </div>
