...
- [x] Fix logo visibility: replaced image-based logo with pixel-perfect vector SVG component (`Logo.tsx`) matching the official PersonaMail brand design (stylized P shell, 3 chat dots, cyan envelope fold gradient) for crisp, non-pixelated rendering across all screen resolutions and Light/Dark themes.
