...
- [x] Fix Groq model decommissioned / 404 error: configured configurable `GROQ_MODEL` setting in `backend/core/config.py` and implemented resilient automatic candidate model fallback chain (`openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `groq/compound`, `groq/compound-mini`) in `backend/services/groq_service.py` to eliminate 503 errors when Groq alters available model IDs.
